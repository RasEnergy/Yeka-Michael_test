		...
		const result = await prisma.$transaction(
			async (tx) => {
				...
				if (!existingStudent) {
					...
					// 			| "NEW_STUDENT",
					// 		enrollmentDate:
					// 			studentData.studentType === "NEW_STUDENT" ? new Date() : null,
				...
				// Calculate fees based on student type and payment option
				let registrationFee = 1000; // Base registration fee
				const additionalFee = Number(studentData.additionalFee) || 0;
				// const additionalFee = 0;

				// For regular students, registration fee might be different
				// if (studentData.studentType === "REGULAR_STUDENT") {
				// 	registrationFee = 500; // Reduced fee for returning students
				// }

				const totalAmount = registrationFee + additionalFee;
				...
				const registration = await tx.registration.create({
					data: {
						...
						registrationFee,
						additionalFee: additionalFee > 0 ? additionalFee : null,
						totalAmount,
						paymentDueDate,
					},
				});
...
// 			where: { branchId: studentData.branchId },
// 		});
// 		const studentId = `${branch.code}${currentYear}${String(
// 			studentCount + 1
// 		).padStart(4, "0")}`;
...
// 			where: { branchId: studentData.branchId },
// 		});
// 		const registrationNumber = `REG-${branch.code}-${currentYear}-${String(
// 			registrationCount + 1
// 		).padStart(4, "0")}`;
...
// 					registrationNumber,
// 					status: "PENDING_PAYMENT",
// 					registrationFee: 1000, // Base registration fee
// 					totalAmount: 1000, // Will be updated when payment option is selected
// 					paymentDueDate,
// 				},
// 			});
...
// 									parent.id
// 								);
// 								updateData.photo = parentPhotoUrl;
// 							} catch (error) {
// 								console.error("Parent photo upload error:", error);
// 							}
// 						}
...
// 						data: {
// 							email:
// 								parentData.parentEmail || `${parentData.parentPhone}@temp.com`,
// 							password: parentPassword,
// 							firstName: parentData.parentFirstName,
// 							lastName: parentData.parentLastName,
// 							phone: parentData.parentPhone,
...
// 							userId: parentUser.id,
// 							occupation: parentData.parentOccupation,
// 							workplace: parentData.parentWorkplace,
// 							address: parentData.parentAddress,
// 							photo: parentPhotoUrl,
// 						},
// 					});
...
// 		return NextResponse.json({
// 			message:
// 				"Student registered successfully. Payment required to complete registration.",
// 			student: {
// 				id: result.student.id,
// 				studentId: result.student.studentId,
// 				studentType: result.student.studentType,
...
// 			if (studentData.phone) await validateUniquePhone(studentData.phone);
// 			if (parentData.parentPhone)
// 				await validateUniquePhone(parentData.parentPhone);
// 		} catch (error) {
// 			if (error instanceof ValidationError) {
// 				return NextResponse.json({ error: error.message }, { status: 400 });
// 			}
